...
export interface KaspaSignTransactionRow {
  ...
  value: string;
  subValue?: string;
  /** Optional tooltip / info icon shown next to the label */
  showInfo?: boolean;
  /** Title shown in the InfoSheet when the info icon is tapped */
  infoTitle?: string;
  /** Description shown in the InfoSheet when the info icon is tapped */
  ...
  return (
    <>
      ...
      <InfoSheet
        ...
        title={activeInfoRow?.infoTitle ?? ""}
        description={activeInfoRow?.infoDescription ?? ""}
      />
    </>
  );
};
